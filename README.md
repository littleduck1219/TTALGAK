...
- Local vertical drag maps exactly to `angle=clamp(45 + 20×dy/48, 25, 65)` in y-up points. Reverse tangent projection maps power with a 6pt dead zone and 160pt maximum: `r=max(0,-dot(D,t))`, `power=clamp((r-6)/154,0,1)`.
- Power latch: the moment a valid drag first crosses the 6pt reverse dead zone, the current angle's tangent is frozen. From then on power reads only the displacement component along that frozen axis (`r=-dx/tangent.x`, same `clamp((r-6)/154,0,1)`): vertical angle-only moves change the launch angle but leave power exactly unchanged; only continued pull along the frozen reverse axis raises (or relaxes) power, reaching max at a 160pt pull. Before the dead zone is crossed, angle changes never latch power. `mouseUp` launches the latest angle with the latched power.
