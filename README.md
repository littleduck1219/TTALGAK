...
- Physical full power is a fixed `160pt` reverse pull. With the `6pt` dead zone, `r=max(0,-dot(D,t))` and `power=clamp((r-6)/154,0,1)`. Power needs no display geometry or edge data.
...
- Visible pull uses the substantial fixed mapping `tension=clamp(rawPull/160×160,0,160)pt`, hidden inside the 6pt dead zone. An `80pt` raw pull shows `80pt` tension and approximately half power; a `160pt` raw pull shows the full `160pt` tension and full power. Vertical angle-only moves preserve raw pull, visible tension, and power together. The hand/launch `P0` never moves during aiming.
