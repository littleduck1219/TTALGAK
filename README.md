...
- Local vertical drag maps exactly to an asymmetric `10…65°` range around the `45°` center in y-up points: `angle=clamp(45 + 35×dy/48, 10, 65)` for `dy<0` and `angle=clamp(45 + 20×dy/48, 10, 65)` for `dy≥0`, so `dy=-48 → 10°`, `dy=0 → 45°`, `dy=+48 → 65°`. Reverse tangent projection maps power with a 6pt dead zone and 320pt maximum: `r=max(0,-dot(D,t))`, `power=clamp((r-6)/314,0,1)`.
...
- Visible pull: the frozen launch retains the raw frozen-axis pull distance (`rawPull`) separately from normalized power, and the tension line renders it proportionally: `tension = clamp(rawPull/320 × 160, 0, 160)pt`, hidden inside the 6pt dead zone. A full 320pt pull shows a 160pt tension line and a 160pt pull shows 80pt. Vertical angle-only moves preserve raw pull, visible tension, and power together; only further pull along the frozen reverse axis grows all three. The hand/launch `P0` never moves during aiming, so tension is the only visual pull indicator and flight starts with no hand-to-spear discontinuity.
