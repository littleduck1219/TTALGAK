...
public struct LocalGesture: Equatable {
    ...
    public mutating func move(to point: PresentationPoint, isInsideInput _: Bool, start: PresentationPoint = PresentationPoint(x: 0, y: 0)) -> FrozenLaunch? {
        ...
        // so vertical (angle-only) moves can never shrink or grow latched power or raw pull.
        let rawPull = latchedTangent.map { max(0, -displacement.x / $0.x) } ?? 0
        let value = FrozenLaunch(angleDegrees: angle, power: DragLaunch.power(rawPull: rawPull), rawPull: rawPull, start: start)
        launch = value
        return value
    }
    public mutating func release(isInsideInput _: Bool) -> FrozenLaunch? {
        defer { down = nil; latchedTangent = nil; launch = nil }
        return launch
    }
    public mutating func cancel() {
        down = nil; latchedTangent = nil; launch = nil
    }
}
